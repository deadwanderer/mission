...
#define m_alloc_struct(arena, type) \
  (type*)m_alloc(arena, sizeof(type), alignof(type))
#define m_alloc_array(arena, count, type) \
  (type*)m_alloc(arena, (count) * sizeof(type), alignof(type))

#define m_alloc_struct_nozero(arena, type) \
  (type*)m_alloc_nozero(arena, sizeof(type), alignof(type))
#define m_alloc_array_nozero(arena, count, type) \
  (type*)m_alloc_nozero(arena, (count) * sizeof(type), alignof(type))

#define m_copy_struct(arena, src) m_copy(arena, src, sizeof(*(src)))
#define m_copy_array(arena, src, count) \
  m_copy(arena, src, (count) * sizeof(*(src)))
...
#define m_scoped(ARENA)                                               \
  for (arena_marker_t __marker = m_get_marker(ARENA); __marker.arena; \
       m_reset_to_marker(ARENA, __marker), __marker.arena = NULL)

void* m_bootstrap_(size_t size, size_t align, size_t arena_offset);
#define m_bootstrap(type, arena) \
  m_bootstrap_(sizeof(type), alignof(type), offsetof(type, arena))
