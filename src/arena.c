...
void* m_alloc_nozero(arena_t* arena, size_t size, size_t align) {
  if (!arena->buffer) {
    ASSERT_MSG(false, "Arena must have a memory buffer.");
  ...
  if (ALWAYS(m_size_remaining_for_align(arena, align) >= size)) {
    ...
    if (result + size > arena->committed) {
      size_t to_commit = align_forward(result + size - arena->committed,
                                       ARENA_COMMIT_CHUNK_SIZE);
...
wchar_t* m_alloc_string16(arena_t* arena, size_t size) {
  return m_alloc(arena, sizeof(u16) * size, DEFAULT_STRING_ALIGN);
}
...
void m_pre_commit(arena_t* arena, size_t size) {
  ...
  size_t to_commit = align_forward(arena->at + size - arena->committed,
                                   ARENA_COMMIT_CHUNK_SIZE);
  // vm_commit(arena->committed, to_commit);
...
void m_reset_and_decommit(arena_t* arena) {
  if (arena->owns_memory) {
    char* decommit_from =
        arena->buffer + ARENA_DEFAULT_COMMIT_PRESERVE_THRESHOLD;
