...
#define ASSERT(expr)                                                           \
  ((expr)                                                                      \
       ? true                                                                  \
       : (fatal_error(__LINE__, strlit(__FILE__), "Assertion failed: " #expr), \
          false))
#define ASSERT_MSG(expr, msg, ...) \
  ((expr)                          \
       ? true                      \
       : (fatal_error(__LINE__, strlit(__FILE__), msg, ##__VA_ARGS__), false))
...
#define INVALID_DEFAULT_CASE                      \
  default: {                                      \
    FATAL_ERROR("Reached invalid default case!"); \
  } break;
