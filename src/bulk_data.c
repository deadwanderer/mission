...
#define BULK_DATA_RESERVE_SIZE MB(128)
...
static size_t bd_stride(bulk_t* bd) {
  return align_forward(
      MAX(sizeof(free_item_t), bd->item_size) + sizeof(bulk_item_t), bd->align);
}

static u32 index_from_bulk_item(bulk_t* bd, bulk_item_t* bulk_item) {
  return (u32)(((char*)bulk_item - bd->buffer) / bd_stride(bd));
}
...
static void bd_init(bulk_t* bd) {
  ASSERT_MSG(bd->item_size,
             "BULK DATA INITIALIZATION FAILURE: ITEM SIZE IS ZERO");

  // if (bd->flags & BULK_FLAGS_CONCURRENT) {
  //   mutex_lock(bd->lock);
  // }
  if (ALWAYS(!bd->buffer)) {
    bd->buffer = calloc(BULK_DATA_RESERVE_SIZE, sizeof(char));
    // vm_commit(bd->buffer, BULK_DATA_COMMIT_SIZE);

    bd->watermark = bd_stride(bd);
  }
...
void* bd_add(bulk_t* bd) {
  ...
  if (sentinel->next != 0) {
    ...
    // bool new_chunk = (align_forward(new_size, BULK_DATA_COMMIT_SIZE) >
    //                   align_forward(bd->watermark, BULK_DATA_COMMIT_SIZE));
    // if (new_chunk) {
    //   vm_commit(bd->buffer + align_backward(new_size, BULK_DATA_COMMIT_SIZE),
    //             BULK_DATA_COMMIT_SIZE);
    // }
...
void* bd_get(bulk_t* bd, resource_handle_t handle) {
  if (NEVER(!bd->buffer)) {
    FATAL_ERROR("Bulk data not initialized!");
  }
...
resource_handle_t bd_get_handle(bulk_t* bd, void* item) {
  ...
  u32 index = index_from_bulk_item(bd, bulk_item);

  if (NEVER(index < 0)) {
    return NULL_RESOURCE_HANDLE;
  }
  if (NEVER(index > count)) {
    return NULL_RESOURCE_HANDLE;
  }
